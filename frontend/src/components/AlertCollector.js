...
export const AlertCollector = () => {
    ...
    const callApi = async () => {
        setIsLoading(true)
        setSearchText('')
        const response = await getLogsByServiceName('alertcollector')
        if (response.status === 200) {
            let AlertCollector = response.data.AlertCollector;
            if (AlertCollector !== null) {
                setLogs(AlertCollector);
                setMatchedLogLines(AlertCollector);
                setIsLoading(false)
            } else {
                setIsLoading(false)
            }
        ...
        <div className={classes.root}>
            <div className={classes.headingContainer}>
                <div style={{ display: 'flex' }}>
                    <Typography style={{ textAlign: 'center', fontWeight: '900', color: "#3f51b5" }} variant='h6'>AlertCollector Logs ({matchedLogLines.length} lines)
                    </Typography>
                    {
                        isLoading ?
                            <CircularProgress
