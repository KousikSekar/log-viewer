...
export const CorrelationEngine = () => {
    ...
    const callApi = async () => {
        setIsLoading(true)
        setSearchText('')
        const response = await getLogsByServiceName('correlationengine')
        if (response.status === 200) {
            let CorrelationEngine = response.data.CorrelationEngine;
            if (CorrelationEngine !== null) {
                setLogs(CorrelationEngine);
                setMatchedLogLines(CorrelationEngine);
                setIsLoading(false)
            } else {
                setIsLoading(false)
            }
        ...
        <div className={classes.root}>
            <div className={classes.headingContainer}>
                <div style={{ display: 'flex' }}>
                    <Typography style={{ textAlign: 'center', fontWeight: '900', color: "#3f51b5" }} variant='h6'>CorrelationEngine Logs ({matchedLogLines.length} lines)
                    </Typography>
                    {
                        isLoading ?
                            <CircularProgress
