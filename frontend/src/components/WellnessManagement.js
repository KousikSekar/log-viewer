...
export const WellnessManagement = () => {
    ...
    const callApi = async () => {
        setIsLoading(true)
        setSearchText('')
        const response = await getLogsByServiceName('wellnessmanagement')
        if (response.status === 200) {
            let WellnessManagement = response.data.WellnessManagement;
            console.log(WellnessManagement)
            if (WellnessManagement !== null) {
                setLogs(WellnessManagement);
                setMatchedLogLines(WellnessManagement);
                setIsLoading(false)
            } else {
                setIsLoading(false)
            }
        ...
        <div className={classes.root}>
            <div className={classes.headingContainer}>
                <div style={{ display: 'flex' }}>
                    <Typography style={{ textAlign: 'center', fontWeight: '900', color: "#3f51b5" }} variant='h6'>WellnessManagement Logs ({matchedLogLines.length} lines)
                    </Typography>
                    {
                        isLoading ?
                            <CircularProgress
