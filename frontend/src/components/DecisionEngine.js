...
export const DecisionEngine = () => {
    ...
    const callApi = async () => {
        setIsLoading(true)
        setSearchText('')
        const response = await getLogsByServiceName('decisionengine')
        if (response.status === 200) {
            let DecisionEngine = response.data.DecisionEngine;
            if (DecisionEngine !== null) {
                setLogs(DecisionEngine);
                setMatchedLogLines(DecisionEngine);
                setIsLoading(false)
            } else {
                setIsLoading(false)
            }
        ...
        <div className={classes.root}>
            <div className={classes.headingContainer}>
                <div style={{ display: 'flex' }}>
                    <Typography style={{ textAlign: 'center', fontWeight: '900', color: "#3f51b5" }} variant='h6'>DecisionEngine Logs ({matchedLogLines.length} lines)
                    </Typography>
                    {
                        isLoading ?
                            <CircularProgress
