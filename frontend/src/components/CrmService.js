...
export const CrmService = () => {
    ...
    const callApi = async () => {
        setIsLoading(true)
        setSearchText('')
        const response = await getLogsByServiceName('crmservice')
        if (response.status === 200) {
            let CrmService = response.data.CrmService;
            if (CrmService !== null) {
                setLogs(CrmService);
                setMatchedLogLines(CrmService);
                setIsLoading(false)
            } else {
                setIsLoading(false)
            }
        ...
        <div className={classes.root}>
            <div className={classes.headingContainer}>
                <div style={{ display: 'flex' }}>
                    <Typography style={{ textAlign: 'center', fontWeight: '900', color: "#3f51b5" }} variant='h6'>CrmService Logs ({matchedLogLines.length} lines)
                    </Typography>
                    {
                        isLoading ?
                            <CircularProgress
