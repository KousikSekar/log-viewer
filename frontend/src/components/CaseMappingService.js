...
export const CaseMappingService = () => {
    ...
    const callApi = async () => {
        setIsLoading(true)
        setSearchText('')
        const response = await getLogsByServiceName('casemappingservice')
        if (response.status === 200) {
            let CaseMappingService = response.data.CaseMappingService;
            if (CaseMappingService !== null) {
                setLogs(CaseMappingService);
                setMatchedLogLines(CaseMappingService);
                setIsLoading(false)
            } else {
                setIsLoading(false)
            }
        ...
        <div className={classes.root}>
            <div className={classes.headingContainer}>
                <div style={{ display: 'flex' }}>
                    <Typography style={{ textAlign: 'center', fontWeight: '900', color: "#3f51b5" }} variant='h6'>CaseMappingService Logs ({matchedLogLines.length} lines)
                    </Typography>
                    {
                        isLoading ?
                            <CircularProgress
...
export default CaseMappingService;