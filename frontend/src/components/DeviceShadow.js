...
export const DeviceShadow = () => {
    ...
    const callApi = async () => {
        setIsLoading(true)
        setSearchText('')
        const response = await getLogsByServiceName('deviceshadow')
        if (response.status === 200) {
            let DeviceShadow = response.data.DeviceShadow;
            if (DeviceShadow !== null) {
                setLogs(DeviceShadow);
                setMatchedLogLines(DeviceShadow);
                setIsLoading(false)
            } else {
                setIsLoading(false)
            }
        ...
        <div className={classes.root}>
            <div className={classes.headingContainer}>
                <div style={{ display: 'flex' }}>
                    <Typography style={{ textAlign: 'center', fontWeight: '900', color: "#3f51b5" }} variant='h6'>DeviceShadow Logs ({matchedLogLines.length} lines)
                    </Typography>
                    {
                        isLoading ?
                            <CircularProgress
