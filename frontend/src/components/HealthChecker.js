...
export const HealthChecker = () => {
    ...
    const callApi = async () => {
        setIsLoading(true)
        setSearchText('')
        const response = await getLogsByServiceName('healthchecker')
        if (response.status === 200) {
            let Healthchecker = response.data.Healthchecker;
            if (Healthchecker !== null) {
                setLogs(Healthchecker);
                setMatchedLogLines(Healthchecker);
                setIsLoading(false)
            } else {
                setIsLoading(false)
            }
        ...
        <div className={classes.root}>
            <div className={classes.headingContainer}>
                <div style={{ display: 'flex' }}>
                    <Typography style={{ textAlign: 'center', fontWeight: '900', color: "#3f51b5" }} variant='h6'>Healthchecker Logs ({matchedLogLines.length} lines)
                    </Typography>
                    {
                        isLoading ?
                            <CircularProgress
